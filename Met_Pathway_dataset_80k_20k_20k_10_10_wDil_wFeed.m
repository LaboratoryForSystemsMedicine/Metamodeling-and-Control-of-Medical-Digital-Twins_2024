function z=Met_Pathway_matlabv2_datset_comDil10_wC(Outside_Feed)
%v2 implementing the changes from the ODE:
%changed the sequence in the output from 'S','P','Q','T','R' to 'S','P','Q','R','T'
...
for k=1:par.addnum
    ...
    while time<par.maxtime
        ...
        if isfinite(par.maxtime)
            ...
            output(time+1,:)=[time,sum(SubS(:,1)>0),sum(SubP(:,1)>0),sum(SubQ(:,1)>0),sum(SubR(:,1)>0),sum(SubT(:,1)>0)];
...
z1=sum(Noutput(1:50001,5+1)*0.0005)+sum(Noutput(1:50001,4+1)*0.0005);
z2=sum(Noutput(1:50001,1+1)*0.0005);
z=z2/z1;
end
