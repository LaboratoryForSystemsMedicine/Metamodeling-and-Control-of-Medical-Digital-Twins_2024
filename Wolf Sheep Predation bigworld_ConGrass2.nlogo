...
to go
  ...
  if ticks >= 1000 [
    ;ask wolves [if random-float 1000 < 5 [ die ]   ] ; 1573  1811
    ;ask sheep  [ if random-float 1000 < 5  [ die ]   ] ;  251  294
    ask patches [if pcolor = green [if random-float 1000 < 20 [ set pcolor brown]]]
   ]
  ...
  if model-version = "sheep-wolves-grass" [ ask patches [ grow-grass ] ]
   ...
   if ticks >= 3000 [
    export-plot "populations" (word "D:/GSW/ConGrass2/results" random 250000 ".csv")
    setup]
